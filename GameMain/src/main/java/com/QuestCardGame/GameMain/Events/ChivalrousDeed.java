...
public class ChivalrousDeed extends EventEffect {
	...
	public boolean eventBehavior() {
		...
		for (int i = 1; i < game.getNumPlayers(); i++) {
			if (game.getPlayer(i).getRank() > leastRank)
				continue;
			else if (game.getPlayer(i).getRank() == leastRank)
		...
		for (int i = 1; i < leastRankedPlayers.size(); i++) {
			if (leastRankedPlayers.get(i).getNumShields() > leastShield)
				continue;
			else if (leastRankedPlayers.get(i).getNumShields() == leastShield)
